...
-- Candidate bodies at one directory, best first. Decision 10a.1 puts the body
-- under App/, so App/ is tried before the directory itself at every candidate
-- root. That ordering is load bearing for the action-path fallback below: the
-- action path names the Screen Reader entrypoint at the package root, and the
-- file called ReaAssist.lua sitting beside it there is the launcher, not the
-- body. Running the launcher from here would start the whole bootstrap
-- sequence again inside a launch that is already past it.
local function main_candidates(dir)
  return {
    join_path(dir, "mbriggs-reaper", "ReaAssist", "App", "ReaAssist.lua"),
    join_path(dir, "mbriggs-reaper", "ReaAssist", "ReaAssist.lua"),
    join_path(dir, "App", "ReaAssist.lua"),
    join_path(dir, "ReaAssist.lua"),
  }
end
...
local function find_main_path(action_path)
  ...
  for _, dir in ipairs(dirs) do
    for _, candidate in ipairs(main_candidates(dir)) do
      if file_exists(candidate) then
        return candidate, dir_name(candidate)
      end
    end
  end
...
-- The two roots this file needs, same split as the body's RA.APP_DIR and
-- RA.PACKAGE_DIR: app_dir holds the body, and its parent holds Data/ when
-- the layout is the App/ one. Reading them off the body that was actually
-- found keeps the two files agreeing about where they are.
--
-- The App segment is stripped only when the directory above holds a
-- marker-verified permanent launcher, which is what makes it a package root.
-- A flat install sitting in a directory that happens to be called App, with
-- any unrelated ReaAssist.lua above it, would otherwise put Data/ in somebody
-- else's folder: existence under one of those two names is not evidence, so
-- the check reads a bounded prefix and looks for the marker the generated
-- launchers carry in their header. Local copy of the rule; the master copy is
...
local function package_dir_for(dir)
  local parent, leaf = tostring(dir or ""):match("^(.*[\\/])([^\\/]+)[\\/]$")
  if parent and leaf == "App" and holds_launcher(parent) then
    return parent
  end
  return dir or ""
end
...
local package_dir = package_dir_for(script_dir)
...
local function osara_note_path()
  if package_dir == "" then return nil end
  local temp_dir = package_dir .. "Data" .. sep .. "Temp"
...
function ScreenReader.payload_jsfx_status(payload)
  local msg = ScreenReader.payload_jsfx_message(payload)
  local status = tostring((msg and msg.jsfx_status) or "")
  if msg and Code.jsfx_add_is_applied(msg)
      and not Code.jsfx_add_can_undo(msg)
      and msg._jsfx_status_final ~= true then
    return ScreenReader.t("jsfx.undo_unavailable_status", {
      status = status,
    }, status
...
function ScreenReader.payload_can_undo(payload)
  if ScreenReader.payload_is_jsfx(payload) then
    ...
    return payload.undo_sent ~= true
      and Code.jsfx_add_can_undo(msg)
  end
  return payload and payload.can_undo == true
end

function ScreenReader.payload_can_request_lua(payload, request_active)
  if request_active or not ScreenReader.payload_is_typed_action(payload) then
    return false
  end
  if payload.typed_action_lua_generation_pending == true
      or payload.typed_action_lua_requested == true then
    return false
  end
  if payload.has_applied_edit == true then
    return ScreenReader.payload_can_undo(payload)
  end
  return true
end
...
function ScreenReader.refresh_actions(opts)
  ...
  ScreenReader.set_button_disabled(ui.ids.request_lua,
    not ScreenReader.payload_can_request_lua(payload, request_active))
  ScreenReader.set_button_disabled(ui.ids.apply_plan,
    request_active
      or not ScreenReader.payload_is_typed_action(payload)
      or not has_code
      or (payload and payload.has_applied_edit == true))
...
function ScreenReader.add_latest_jsfx_to_selected_tracks(next_view)
  ...
  if Code.jsfx_add_is_applied(msg) then
    local status = ScreenReader.payload_jsfx_status(payload)
    ScreenReader.set_status(ScreenReader.t(
      "jsfx.add_already_applied_status", { status = status },
      "The JSFX was not added again. " .. status), true)
    return false
  end

  local action_project = Code.active_project()
  if not Code.jsfx_add_project_is_supported(action_project) then
    ScreenReader.set_status(ScreenReader.t(
      "jsfx.project_safety_unavailable", nil,
      "ReaAssist could not verify the active project, so the JSFX was not added."),
      true)
    return false
  end

  local sel_count = reaper.CountSelectedTracks
    and reaper.CountSelectedTracks(action_project) or 0
  if sel_count <= 0 then
  ...
  if not Code.project_is_active(action_project) then
    ScreenReader.set_status(ScreenReader.t(
      "jsfx.project_safety_unavailable", nil,
      "ReaAssist could not verify the active project, so the JSFX was not added."),
      true)
  ...
  sel_count = reaper.CountSelectedTracks
    and reaper.CountSelectedTracks(action_project) or 0
  if sel_count <= 0 then
    ScreenReader.set_status(ScreenReader.t("a11y.sr.add_jsfx_no_tracks", nil,
      "Select one or more tracks in REAPER first, then add the JSFX."), true)
  ...
  for t = 0, sel_count - 1 do
    tracks[#tracks + 1] = reaper.GetSelectedTrack(action_project, t)
  end

  local added_n, add_reason = Code.add_jsfx_to_tracks(
    action_project, tracks, fx_name)
...
function ScreenReader.undo_latest_generated_action(next_view)
  ...
  local msg = ScreenReader.payload_jsfx_message(payload)
  if msg then
    local ok, reason = Code.undo_jsfx_add(msg)
    local status
    if ok then
      msg.jsfx_status = nil
      msg._jsfx_status_final = nil
      ...
      msg.auto_ran = false
      status = ScreenReader.t("a11y.sr.jsfx_undo_done", nil,
        "Undo sent. JSFX add was reverted.")
    elseif reason == "failed" then
      local prior_status = tostring(msg.jsfx_status or "")
      status = ScreenReader.t("jsfx.undo_failed_status", {
        status = prior_status,
      }, prior_status
        .. " REAPER could not undo this JSFX add. Review the project and Undo history before trying again.")
      msg.jsfx_status = status
      msg._jsfx_status_final = true
    else
      status = ScreenReader.payload_jsfx_status(payload)
    end
    ScreenReader.set_status(status, true)
...
-- ---------------------------------------------------------------------------
-- Uninstall (Distribution Unification Plan section 11, B2.4a)
-- ---------------------------------------------------------------------------
-- Every word of both screens comes from Updater.uninstall_copy and
-- Updater.uninstall_result_copy, the same two functions the visual surface
-- reads, so this entry point cannot drift into saying something the visual one
-- does not. Until B2.4a a Screen Reader user could not uninstall at all except
-- by opening the visual interface.

function ScreenReader.uninstall_available()
  return Updater ~= nil and type(Updater.uninstall_plan) == "function"
    and type(Updater.uninstall_execute) == "function"
end

function ScreenReader.open_uninstall_confirm()
  if not ScreenReader.uninstall_available() then
    ScreenReader.set_status(ScreenReader.t("a11y.sr.uninstall_unavailable",
      nil, "Uninstall is not available in this build."), true)
    return
  end
  if AppController.request_is_active() then
    ScreenReader.set_status(ScreenReader.t("a11y.sr.uninstall_blocked_active",
      nil, "Cancel or finish the active request before uninstalling."), true)
    return
  end
...
-- The gate's own answers come first when it has any, the way the visual
-- surface puts them first: a user who is going to be refused reads the refusal
-- instead of a description of a removal that is not going to happen.
function ScreenReader.uninstall_blocked()
  local plan = S and S._screen_reader_uninstall_plan
  if type(plan) ~= "table" then return false end
  return #(plan.blockers or {}) > 0
end

function ScreenReader.uninstall_confirm_text()
  local plan = S and S._screen_reader_uninstall_plan
  ...
  plan.remove_user_data = S._screen_reader_uninstall_remove_data and true
    or false
  return tostring(Updater.uninstall_copy(plan) or "")
end

function ScreenReader.uninstall_result_text()
  local result = S and S._screen_reader_uninstall_result
  if type(result) ~= "table" then return "" end
  return tostring(Updater.uninstall_result_copy(result) or "")
end

function ScreenReader.set_uninstall_remove_data(checked)
  S._screen_reader_uninstall_remove_data = checked and true or false
  ScreenReader.set_status_after_rebuild(
    S._screen_reader_uninstall_remove_data
      and ScreenReader.t("a11y.sr.uninstall_data_on", nil,
        "Your ReaAssist data will be removed too.")
      or ScreenReader.t("a11y.sr.uninstall_data_off", nil,
        "Your ReaAssist data will be left where it is."), true)
  ScreenReader.open_view("uninstall_confirm")
end

function ScreenReader.confirm_uninstall()
  local plan = S and S._screen_reader_uninstall_plan
  if type(plan) ~= "table" then
    ScreenReader.cancel_uninstall()
    return
  end
  plan.remove_user_data = S._screen_reader_uninstall_remove_data and true
    or false
  S._screen_reader_uninstall_plan = nil
  S._screen_reader_uninstall_result = Updater.uninstall_execute(plan)
  ScreenReader.set_status_after_rebuild(ScreenReader.t(
    "a11y.sr.uninstall_finished", nil,
    "The uninstall has finished. Read the report, then close ReaAssist."),
    true)
  ScreenReader.open_view("uninstall_result")
end

function ScreenReader.cancel_uninstall()
  S._screen_reader_uninstall_plan = nil
  S._screen_reader_uninstall_result = nil
  S._screen_reader_uninstall_remove_data = false
  ScreenReader.set_status_after_rebuild(ScreenReader.t(
    "a11y.sr.uninstall_cancelled", nil,
    "Uninstall cancelled. Nothing was removed."), true)
  ScreenReader.open_view("settings")
end

function ScreenReader.copy_uninstall_report()
  local ok = AppController.copy_text(ScreenReader.uninstall_result_text())
  ScreenReader.set_status(ok and ScreenReader.t("a11y.sr.uninstall_copied",
    nil, "Uninstall report copied to clipboard.")
...
function ScreenReader.shortcut_run_code()
  ...
  if ScreenReader.payload_is_typed_action(payload)
      and payload.has_code == true
      and payload.has_applied_edit ~= true then
...
function ScreenReader.shortcut_back()
  ...
  elseif view == "api_keys"
    ...
    elseif view == "uninstall_confirm" then
      ScreenReader.cancel_uninstall()
    else
      ScreenReader.open_view("settings")
    end
...
function ScreenReader.view_title()
  ...
  if view == "uninstall_confirm" then
    return ScreenReader.t("settings.uninstall.heading", nil,
      "Remove ReaAssist?")
  end
  if view == "uninstall_result" then
    return ScreenReader.t("settings.uninstall.result_title", nil, "Uninstall")
  end
...
function ScreenReader.build_settings_ui()
  ...
  ui.ids.settings_summary = reagirl.Label_Add(nil, nil,
    ScreenReader.wrap_text(ScreenReader.settings_summary_display_text(), 84),
  ...
  ui.ids.check_updates = reagirl.Button_Add(nil, nil, 10, 5,
    ...
    ScreenReader.t("a11y.sr.check_updates.meaning_v2", nil,
      "Checks for ReaAssist updates and install repairs. This works whether "
  ...
  ui.ids.factory_reset = reagirl.Button_Add(nil, nil, 10, 5,
    ...
    "factory_reset")
  if ScreenReader.uninstall_available() then
    ui.ids.uninstall = reagirl.Button_Add(nil, nil, 10, 5,
      ScreenReader.t("settings.adv.uninstall.label", nil,
        "Uninstall ReaAssist"),
      ScreenReader.t("a11y.sr.uninstall.meaning", nil,
        "Removes ReaAssist from this REAPER install. Your data is a separate choice inside."),
      function() ScreenReader.open_uninstall_confirm() end,
      "uninstall")
  end
  ...
  ui.ids.debug_logging = reagirl.Checkbox_Add(nil, nil,
    ...
    "debug_logging")
  ScreenReader.next_line_for_large_text()
  -- Plan section 7 (work package B2.5). OFF means "I update manually", not
  -- "I update through ReaPack", and the Check Updates button on this same
  -- view stays a one-shot that Updater.manual_check runs without reading this
  -- preference. The meaning also carries the B2.2a disclosure: the ungated,
  -- damage-gated manifest repair lane fetches repair data and converges on the
  -- current release with this off, while an intact install makes no contact
  -- from that lane. Rounds 64 and 65 corrected two overstatements here
  -- (B2.5a, B2.5b): the automatic check is one ATTEMPT after the first
  -- request, not a promised check, because the trigger marks the session
  -- fired before check_start runs and check_start declines while the
  -- updater or a font or language download is busy; and "no contact" is
  -- AUTOMATIC contact, since the button two sentences earlier contacts the
  -- server whenever the user asks.
  ui.ids.update_check = reagirl.Checkbox_Add(nil, nil,
    ScreenReader.t("settings.pref.update_check.label_v2", nil,
      "Check for updates automatically"),
    ScreenReader.t("a11y.sr.update_check.meaning_v2", nil,
      "On, after your first request in a session, ReaAssist tries one "
      .. "automatic check for a newer version. Off, you update manually, and "
      .. "Check Updates still "
      .. "works every time you choose it. One exception: if ReaAssist finds "
      .. "its own record of the files it installed missing or damaged, it "
      .. "downloads a fresh copy of that record even with this off, and "
      .. "brings the install to the current release through the normal "
      .. "update path, which still asks before installing a new version. An "
      .. "undamaged install with this off makes no automatic update contact "
      .. "at all: ReaAssist reaches the update server only when you ask it "
      .. "to."),
...
function ScreenReader.ask_provider_for_local_answer(payload)
  ...
  if not ok or (sent ~= true and S.pending_local_escalation == nil) then
...
function ScreenReader.build_response_ready_ui()
  ...
  local local_provider_available = Net and Net.local_escalation_available
    and Net.local_escalation_available(payload and payload.message) or false
  ...
  if has_code then
    ...
    if typed_action and ScreenReader.payload_can_undo(payload)
        and payload.typed_action_lua_generation_pending ~= true
        and payload.typed_action_lua_requested ~= true then
      ui.ids.request_lua = reagirl.Button_Add(nil, nil, 28, 5,
        ScreenReader.t("typed_actions.request_lua", nil,
          "Request Lua"),
        ScreenReader.t("typed_actions.request_lua_after_apply.tooltip", nil,
    ...
    ui.ids.read_code = reagirl.Button_Add(nil, nil,
      ...
      "read_code")
    if typed_action and payload.has_applied_edit ~= true then
...
function ScreenReader.build_turn_budget_confirm_ui()
  ...
  reagirl.NextLine()
  ui.ids.body = reagirl.Label_Add(nil, nil,
    ScreenReader.wrap_text(body, 82),
  ...
  ui.ids.scope = reagirl.Label_Add(nil, nil,
    ScreenReader.wrap_text(ScreenReader.t("dialog.turn_budget.one_call", nil,
...
function ScreenReader.build_uninstall_confirm_ui()
  S.screen_reader_ui = { ids = {} }
  local ui = S.screen_reader_ui
  ScreenReader.begin_reagirl_ui()

  ui.ids.title = reagirl.Label_Add(18, 18,
    ScreenReader.uninstall_blocked()
      and ScreenReader.t("settings.uninstall.blocked_heading", nil,
        "ReaAssist cannot uninstall right now")
      or ScreenReader.view_title(),
    ScreenReader.t("a11y.sr.uninstall_title.meaning", nil,
      "Confirmation before removing ReaAssist from this REAPER install."),
    false, nil, "uninstall_title")

  local blocked = ScreenReader.uninstall_blocked()
  if not blocked then
    reagirl.NextLine()
    ui.ids.remove_data = reagirl.Checkbox_Add(nil, nil,
      ScreenReader.t("settings.uninstall.remove_data", nil,
        "Also remove my ReaAssist data (settings, keys, chats, logs)"),
      ScreenReader.t("a11y.sr.uninstall_remove_data.meaning", nil,
        "Removes settings, saved keys, custom providers, saved chats, logs and caches as well. This cannot be undone."),
      S and S._screen_reader_uninstall_remove_data == true,
      function(_, checked) ScreenReader.set_uninstall_remove_data(checked) end,
  ...
  if not blocked then
    ui.ids.uninstall = reagirl.Button_Add(nil, nil, 10, 5,
      ScreenReader.t("settings.adv.uninstall.label", nil,
        "Uninstall ReaAssist"),
      ScreenReader.t("a11y.sr.uninstall_confirm.meaning", nil,
        "Removes ReaAssist after confirmation, and reports what went and what stayed."),
      function() ScreenReader.confirm_uninstall() end,
      "uninstall_confirm")
  end
  ui.ids.back = reagirl.Button_Add(nil, nil, 22, 5,
    ScreenReader.back_label("a11y.sr.back_to_settings", "Back to Settings"),
    ScreenReader.t("a11y.sr.back_to_settings.meaning", nil,
      "Returns to the settings screen."),
    function() ScreenReader.cancel_uninstall() end,
    "back_to_settings")
  ui.ids.close = reagirl.Button_Add(nil, nil, 10, 5,
    ScreenReader.close_label(),
    ScreenReader.t("a11y.sr.close.meaning", nil,
  ...
  reagirl.NextLine()
  ui.ids.body = reagirl.Label_Add(nil, nil,
    ScreenReader.wrap_text(ScreenReader.uninstall_confirm_text(), 86),
    ScreenReader.t("a11y.sr.uninstall_body.meaning", nil,
      "What ReaAssist removes, what it leaves alone, and what happens to your data."),
    false, nil, "uninstall_body")
  ...
  local ok = reagirl.Gui_Open("ReaAssist_Screen_Reader_Mode", true,
    ...
    760, 460, 0, nil, nil)
  if ok == 1 then
    ScreenReader.announce(ScreenReader.uninstall_blocked()
      and ScreenReader.t("a11y.sr.uninstall_blocked_opened", nil,
        "ReaAssist cannot uninstall right now. The reason is on this screen. Press F9 to go back.")
      or ScreenReader.t("a11y.sr.uninstall_confirm_opened", nil,
...
function ScreenReader.build_uninstall_result_ui()
  S.screen_reader_ui = { ids = {} }
  local ui = S.screen_reader_ui
  ScreenReader.begin_reagirl_ui()

  ui.ids.title = reagirl.Label_Add(18, 18, ScreenReader.view_title(),
    ScreenReader.t("a11y.sr.uninstall_result_title.meaning", nil,
      "The report of what the uninstall removed and what it left."),
    false, nil, "uninstall_result_title")

  reagirl.NextLine()
  ui.ids.copy = reagirl.Button_Add(nil, nil, 10, 5,
    ScreenReader.t("common.copy", nil, "Copy"),
    ScreenReader.t("a11y.sr.uninstall_copy.meaning", nil,
      "Copies the uninstall report to the clipboard."),
    function() ScreenReader.copy_uninstall_report() end,
    "uninstall_copy")
  ui.ids.close = reagirl.Button_Add(nil, nil, 10, 5,
    ScreenReader.close_label(),
    ScreenReader.t("a11y.sr.close.meaning", nil,
  ...
  reagirl.NextLine()
  ui.ids.body = reagirl.Label_Add(nil, nil,
    ScreenReader.wrap_text(ScreenReader.uninstall_result_text(), 86),
    ScreenReader.t("a11y.sr.uninstall_result_body.meaning", nil,
      "What was removed, what would not go, and how to finish by hand."),
    false, nil, "uninstall_result_body")
  ...
  if ok == 1 then
    ScreenReader.announce(ScreenReader.t(
      "a11y.sr.uninstall_result_opened", nil,
...
function ScreenReader.build_ui()
  ...
  if view == "uninstall_confirm" then
    return ScreenReader.build_uninstall_confirm_ui()
  end
  if view == "uninstall_result" then
    return ScreenReader.build_uninstall_result_ui()
  end
  if view == "visual_switch_confirm" then
...
function ScreenReader.loop()
  ...
  -- The parked copy, dispatched from this loop as well as from the visual one
  -- (Distribution Unification Plan, section 3). The two modes are separate
  -- frames on separate files, and a session that never opened the visual app
  -- would otherwise never park: a Screen Reader user would reach the N+1
  -- transition with no offline restore, which is the user least able to put an
  -- install back by hand. The dispatcher itself decides whether anything runs.
  if Updater and type(Updater.park_dispatch) == "function" then
    local parked, park_err = pcall(Updater.park_dispatch)
    if not parked and Log and Log.line then
...
function AppController.test_active_provider_key()
  ...
  api_keys.start_key_test("direct")
  Net.fire_key_test(p)
  return true
end
...
function AppController.thinking_items()
  ...
  for i, level in ipairs(p.thinking_levels) do
    if PROVIDERS.thinking_idx_allowed(p, m, i) then
      out[#out + 1] = {
        idx = i,
...
function AppController._refresh_attachment_costs(model)
  ...
  for _, att in ipairs(S.attachments or {}) do
    att.cost = MODELS.calc_cost(model, att.tokens or 0, 0, 0, 0)
  end
end
...
function AppController.message_has_applied_typed_action(msg)
...
function AppController.latest_response_payload()
  ...
  return {
    ...
    has_applied_edit = typed_action
      and AppController.message_has_applied_typed_action(msg) or false,
    can_undo = AppController.message_can_undo_generated_action(msg),
    typed_action_lua_generation_pending = msg
      and msg.typed_action_lua_generation_pending == true or false,
